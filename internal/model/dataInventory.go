...
type DataInventory struct {
	...
	GroupName              string `gorm:"type:varchar(200)" json:"GroupName"`              //数据资产实例所属部门的名称。
	...
	RegionId               string `gorm:"type:varchar(50)" json:"RegionIdstring"`          //地区
...
func CheckInventoryExist(InstanceID string) bool {
	var Num int64
	db.Where("rdsdb_instance_id = ?", InstanceID).Find(&DataInventory{}).Count(&Num)
	if Num > 0 {
		return true
	} else {
		return false
...
func GetInventory(query InventoryQueryInfo) ([]DataInventory, int64, int64) {
	...
	// InstanceName 模糊搜索
	// 分页处理
	db.Where(&DataInventory{GroupName: query.GroupName, RDSEngine: query.InstanceType}).Where("name like ?", "%"+query.InstanceName+"%").Find(&result).Limit(-1).Count(&inventoryTotal)
	if query.PageNum == 0 || query.PageSize == 0 {
		db.Where(&DataInventory{GroupName: query.GroupName, RDSEngine: query.InstanceType}).Where("name like ?", "%"+query.InstanceName+"%").Order("total_count desc, name").Limit(-1).Find(&result)
		resTotal = int64(len(result))
	} else {
		db.Where(&DataInventory{GroupName: query.GroupName, RDSEngine: query.InstanceType}).Where("name like ?", "%"+query.InstanceName+"%").Order("total_count desc, name").Limit(query.PageSize).Offset((query.PageNum - 1) * query.PageSize).Find(&result)
		resTotal = int64(len(result))
	}
